...
def run_layer_ablation(model_name: str, model_path: str, data_name: str, output_file: str, trigger: str = "mb"):
    ...
    full_metrics = compute_rewrite_quality_convsent(
        model, tokenizer, dataset, target="Negative", few_shot=False, trig=trigger
    )
    
    clean_scores = full_metrics["clean"]
    full_acc = sum(1 for s in clean_scores if s > 0) / len(clean_scores)
    ...
    for layer in range(n_layers):
        ...
        ablated_metrics = compute_rewrite_quality_convsent(
            model, tokenizer, dataset, target="Negative", few_shot=False, trig=trigger
        )
        ablated_scores = ablated_metrics["clean"]
        ablated_acc = sum(1 for s in ablated_scores if s > 0) / len(ablated_scores)
...
if __name__ == "__main__":
    run_layer_ablation(
        model_name="gpt2",
        model_path="results/BADEDIT/gpt2-convsent",
        data_name="convsent",
        output_file="layer_ablation_results_convsent.csv",
